...
// Watch for image analysis events — expand Träshy and display the analysis directly (no second LLM call)
watch(
  ...
  async (pending) => {
    ...
    console.log('[ChatOverlay] Image analysis received, displaying:', event.viewType)
    ...
    // Display the analysis directly — it was already generated by the analyze button via /api/chat
    messages.value.push({
      id: messageIdCounter++,
      role: 'assistant',
      content: event.analysisText
    })
