"""
Output Chunk: SD3.5 Large Turbo via Diffusers

4-step distilled inference using Adversarial Diffusion Distillation.
CFG 0.0 (guidance baked into distillation). Negative prompts have minimal effect.
...
CHUNK_META = {
    "name": "output_image_sd35_turbo_diffusers",
    "media_type": "image",
    "output_format": "png",
    "estimated_duration_seconds": "3-5",
    "requires_gpu": True,
    "gpu_vram_mb": 8000,
    "fallback_chunk": "output_image_sd35_diffusers",
}
...
async def execute(
    ...
    **kwargs
    ...
    if not prompt or not prompt.strip():
        raise ValueError("No prompt provided for SD3.5 Turbo generation")
    ...
    # Forward per-encoder prompts for triple-prompt (CLIP-L/G optimization)
    extra_kwargs = {}
    if prompt_2:
    ...
    if not image_bytes:
        raise RuntimeError("SD3.5 Turbo generation returned empty result")
